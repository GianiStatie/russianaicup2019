...
class HomebrewGym():
    def __init__(self, host, port, token, os_name, config_path=None, render_env=True):
        self.host        = host
        self.port        = port
        self.token       = token
        ...
        self.g_host    = game_host.make(self.os_name, self.config_path, self.render_env)
        self.g_host.start()
        self.g_client  = game_client.make(self.host, self.port, self.token)

    def reset(self):
        self.g_host.reset()
        self.g_client = game_client.make(self.host, self.port, self.token)
        self.player_view = self.g_client.get_raw_state()
    ...
    def step(self, actions):
        ...
        """
        self.g_client.send_actions(actions)
        self.player_view = self.g_client.get_raw_state()
